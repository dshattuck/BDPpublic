...
function estimate_ERFO_slice(DWI_file, bMatrices, user_options)
%  DWIfile - 4D nifti file name
%  bMatrices - 3 dimensional array of matrices (3x3xN)

disp('Estimating ERFO ODFs in diffusion coordinates...')
...
opt = struct( ...
   ...
   'ERFO_out_dir', fullfile(fileparts(DWI_file), 'ERFO'), ...
...
ERFOodf_output_file_base = fullfile(opt.ERFO_out_dir, fname);
...
outSize = [size(sphericalHarmonicMatrixReg,1), size(dwi.img,1), size(dwi.img,2), size(dwi.img,3)];
...
for nslice = 1:size(dwi.img,3),
   ...
   odf = erfoOdfBasis'*dwimages;
   odf(odf<0) = 0;
   sh_erfo(:,:,nslice,:) = sphericalHarmonicMatrixReg*odf;
end;
clear dwi b0mean

sh_erfo = permute(reshape(sh_erfo,outSize), [2,3,4,1]);

erfo_fid = fopen([ERFOodf_output_file_base '.SH.ERFO.odf'], 'w');
...
% write output files
disp('Writing output ERFO ODF files to disk...')
for k = 1:size(sh_erfo,4) %Need to figure this out
    fname = sprintf('%s.SH.ERFO.%03d.nii.gz', ERFOodf_output_file_base, k);
